...
void Channel::SetMode(Player const* player, std::string const& p2n, bool mod, bool set)
{
    ...
    if (!victim || !IsOn(victim) ||
        ...
        ((!AccountMgr::IsGMAccount(sec) || !AccountMgr::IsGMAccount(newp->GetSession()->GetSecurity())) && player->GetTeamId() != newp->GetTeamId() && !sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_CHANNEL)))
    {
        WorldPacket data;
...
void Channel::SetOwner(Player const* player, std::string const& newname)
{
    ...
    if (!victim || !IsOn(victim) || newp->GetTeamId() != player->GetTeamId() && !sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_CHANNEL))
    {
        WorldPacket data;
