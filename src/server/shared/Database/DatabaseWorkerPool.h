...
class DatabaseWorkerPool
{
    public:
        ...
        _queue(new ACE_Activation_Queue(_mqueue))
        {
            ...
            WPFatal(mysql_get_client_version() >= MIN_MYSQL_CLIENT_VERSION, "ReinsCoreWotlk does not support MySQL versions below 5.1");
        }
        ...
        bool Open(const std::string& infoString, uint8 async_threads, uint8 synch_threads)
        {
            ...
            for (uint8 i = 0; i < async_threads; ++i)
            {
                ...
                if (res) // only check mysql version if connection is valid
                    WPFatal(mysql_get_server_version(t->GetHandle()) >= MIN_MYSQL_SERVER_VERSION, "ReinsCoreWotlk does not support MySQL versions below 5.1");
